...
@implementation KDPlanViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if ([self.navTitle isEqualToString:@"空卡还款计划"]) {
            KDPlanCenterKongKaCell *cell = [KDPlanCenterKongKaCell cellWithTableView:tableView];
            cell.directModel = self.directModel;
            return cell;
        }else{
            KDPlanCenterViewCell *cell = [KDPlanCenterViewCell cellWithTableView:tableView];
            cell.directModel = self.directModel;
            cell.delegate = self;
            return cell;
        }
